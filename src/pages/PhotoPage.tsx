import React, { useState, useEffect, useRef, useCallback } from 'react';
    ...
    // In-memory storage for temporary photo data
    const photoStorage = new Map<string, string>();

    // Exported functions to interact with photoStorage
              ...
              {!photoId ? (
                ...
                <div className="space-y-6 flex flex-col items-center justify-center">
                  <div className='overflow-hidden'>
                    {/* Display a placeholder or a small thumbnail.  Don't display the full image here. */}
                    <img
                      src={getPhoto(photoId)}
                      alt="Captured Thumbnail"
                      className="w-full max-w-xs rounded-lg shadow-lg"
                      style={{
                        maxHeight: '20vh', // Smaller
                        objectFit: 'contain',
                  ...
                  <div className="flex flex-col items-center justify-center gap-4">
                    <QRCodeCanvas value={photoId ? `${window.location.origin}/download/${photoId}` : ''} size={256} level="H" className="rounded-lg" />
                  </div>
